 ...
 */
    ...
    {
        PriorityQueue<NoHuffman> fila =
                new PriorityQueue<NoHuffman>(tamanhoAlfabeto, new ComparadorNoHuffman());
...
/*     private void debugaTabela(char c, String s) throws Exception
    ...
    /*
     * private void imprimeArvore(NoHuffman no) throws IOException { RAF teste = new RAF(prefixo +
     * "testeArvore.db", "rw"); imprimeArvore(no, teste); teste.close(); }
     */

    /*
     * private void imprimeArvore(NoHuffman no, RAF teste) throws IOException { if (no.eFolha() ==
     * true) { teste.writeUTF("true ");
     * 
     * teste.writeChar(no.caractere);
     * 
     * } else { teste.writeUTF("false "); imprimeArvore(no.esq, teste); imprimeArvore(no.dir,
     * teste); } }
     */
    ...
    /**
     * Este método concatena no final da representação da árvore trie o texto compactado, no último
    ...
    /* private void debugaUmByte(char c) throws Exception
    {
        ...
        System.out.println(
                "CHAR " + c + " escrevendo como unsignedByte " + (char) teste.readUnsignedByte());
        teste.close();
